...
int main (int argc, char *argv[]) {
	...
	srcA = (cl_float *) malloc(sizeof(cl_float) * 2*iNumElements);
	...
	if (LINUX) programHandle = fopen("CalculaPI.cl", "r");
	...
	bufferA = clCreateBuffer(context, CL_MEM_READ_ONLY, 2*datasize, NULL, &status);
	...
	status = clEnqueueWriteBuffer(cmdQueue, bufferA, CL_FALSE, 0, 2*datasize, srcA, 0, NULL, NULL);
	...
	ckKernel = clCreateKernel(cpProgram, "CalculaPI", &ciErr);
	...
	ciErr |= clSetKernelArg(ckKernel, 2, sizeof(cl_int), (void*)&iNumElements);
